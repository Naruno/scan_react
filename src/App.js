...
function BlockHistoryList() {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch('https://testnet1.decentranetwork.net:8101/export/block/json');
...
function Decentra_Network_Scan() {
  ...
  useEffect(() => {
    async function fetchData() {
      let data = null
      try {
        const response = await fetch('https://testnet1.decentranetwork.net:8102/status');
